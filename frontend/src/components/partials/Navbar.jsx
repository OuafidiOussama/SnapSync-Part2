...
const Authorized = ({userInfo, logout}) =>{
  const {user} = userInfo
  return (
    <div className='flex h-20 w-96 py-5 items-center justify-around'>
      <span>{user.name}</span>
      <Button label="logout" bg="bg-red-600" hover="hover:bg-red-800" handler={logout}/>
    </div>
  )
}
...
function Navbar() {
  ...
  return (
    <div className="my-5 bg-white shadow-2xl py-3 h-20 rounded-2xl flex items-center justify-between px-16">
        ...
        {userInfo ? <Authorized userInfo={userInfo} logout={logout} /> : <NotAuthorized loading={loading} />}
    </div>
  )
}
